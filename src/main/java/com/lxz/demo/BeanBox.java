...
/**
 * jBeanBox is a micro scale IOC & AOP tool follows BSD-3 license.
 * 
 *@author Yong Zhu
...
public class BeanBox {
	...
	private boolean isValueType = false; // if true mean this bean is a value, not a bean, no need to create an instance for it
	...
	// private static CopyOnWriteArrayList<Advisor> advisorList = new CopyOnWriteArrayList<Advisor>();// Advisors stored in BeanBox's name space
	...
	private BeanBox findAnnotationFieldBox(Class<?> BeanClass, Class<?> annotationClass, Field field) {
		...
		if (!Object.class.equals(annotationClass)) {// @InjectBox(AnnotationClass.class)
			if (BeanBox.class.isAssignableFrom((Class) annotationClass))
				...
				box = ifExistBeanBoxClass(annotationClass.getName() + "$" + fieldClass.getSimpleName() + context.boxIdentity);// case 2
			if (box == null)
				box = ifExistBeanBoxClass(annotationClass.getName() + "$" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1) + context.boxIdentity);// case 3
		} else {// @InjectBox
			box = ifExistBeanBoxClass(fieldClass.getName() + context.boxIdentity);// case 4
			if (box == null) {
				box = ifExistBeanBoxClass(BeanClass.getName() + context.boxIdentity + "$" + fieldClass.getSimpleName() + context.boxIdentity);// case 5
				if (box == null) {
					for (Class<?> configs : context.configClassList) {
						box = ifExistBeanBoxClass(configs.getName() + "$" + fieldClass.getSimpleName() + context.boxIdentity);// case 6
						if (box != null)
							break;
					}
					if (box == null) {
						for (Class<?> configs : context.configClassList) {
							box = ifExistBeanBoxClass(configs.getName() + "$" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1) + context.boxIdentity);// case 7
							if (box != null)
								break;
						}
					}
	...
	public BeanBox setValueType(boolean isValueType) {// if set true, will not create class instance, simple return classOrValue
		this.isValueType = isValueType;
		return this;
	}

	public Object getProperty(String property) {// get the property
		return ((Object[]) (properties.get(property)))[1];
	}
	...
	public BeanBox setSingleTon(boolean isSingleTon) {// Default type is singleTon, all signleTon bean instance will be cached
	...
	public BeanBox setProperty(String property, Object beanBoxInstanceOrValue) {
		...
		else if (beanBoxInstanceOrValue instanceof Class && BeanBox.class.isAssignableFrom((Class) beanBoxInstanceOrValue))
			try {
				properties.put(property, new Object[] { PropertyType.BEAN, createBeanOrBoxInstance((Class) beanBoxInstanceOrValue, context) });
			} catch (Exception e) {
				printAndThrowError(e, "BeanBox setProperty error! property=" + property + " beanBoxInstanceOrValue=" + beanBoxInstanceOrValue);
			}
	...
	public BeanBox setStaticFactory(String property, Class staticFactoryClass, String methodName, Object... args) {// as title
	...
	private void invokeMethodToSetValue(Object bean, Method method, Object... args) {
		try {
			...
			printAndThrowError(e, "BeanBox invokeMethodToSetValue error! bean=" + bean + " method=" + method + " args=" + args);
		}
	}
	...
	private void forceInjectFieldValue(Object bean, Field field, Object... args) {
		try {
			...
			printAndThrowError(e, "BeanBox invokeMethodToSetValue error! bean=" + bean + " field=" + field + " args=" + args);
		}
	}
		...
		for (String property : keys) {
			...
			for (Method method : methods) {
				...
				if (method.getName().equals(setter)) {
					invokeMethodToSetValue(instance, method, (Object[]) properties.get(property));
					found = true;
				}
			}
			if (!found) {
				try {
					...
					forceInjectFieldValue(instance, field, (Object[]) properties.get(property));
				} catch (Exception e) {
					printAndThrowError(e, "BeanBox setInstancePropertyValues error! class=" + this.getClassOrValue() + ", property ='" + property
	 ...
	 * 1) A.class{ @Inject(B.class) C c; ...} will find B.class as BeanBox class and inject to c, if B.class is not beanBox, will find static class "B.CBox.class" in B.class<br/>
	 * 2) A.class{ @Inject C c; ...} will find BeanBox CBox.class in same package of C, if not found, find "ABox.CBox.class" in ABox.class, if not found, find CBox in globalConfigs
	 */
			...
			try {
				if (injectAnnotation != null) {
					...
					if (box == null && injectAnnotation.required())
						printAndThrowError(null, "BeanBox injectAnnotationFields required BeanBox not found! beanClass=" + beanClass + ", field name=" + field.getName());
					if (box == null)
						return;
				...
				printAndThrowError(e, "BeanBox injectAnnotationFields error! beanClass=" + beanClass + " field=" + field.getName());
			}
		}
	}
	...
	private Class[] getObjectClassType(Object... beanArgs) {
		...
		for (int i = 0; i < classes.length; i++) {
			...
			if (BeanBox.class.isAssignableFrom(classes[i])) {
				classes[i] = (Class) (((BeanBox) beanArgs[i]).getClassOrValue());
			}
		}
		return classes;
	}
	...
	public <T> T getBean() {// create the bean or get singleTon bean from cache
		...
		if (BeanBox.class.getName().equals(beanID)) {
			if (this.getClassOrValue() instanceof Class)// use real bean class name & args as bean ID
				beanID = ((Class) this.getClassOrValue()).getName() + (constructorArgs == null ? "" : constructorArgs);
			else
		...
		synchronized (context.signletonCache) {
			...
			if (ifHaveAdvice(context.advisorList, classOrValue)) {
				...
				try {
					...
					if (instance == null)
						printAndThrowError(null, "BeanBox call constructor error! not found match constructor for " + classOrValue);
				} catch (Exception e) {
			...
			if (isSingleTon) {
				...
				if (!isEmptyStr(this.getPreDestory())) {
					try {
						Method predestoryMethod = instance.getClass().getDeclaredMethod(getPreDestory(), new Class[] {});
	...
	private static BeanBox getBoxInstance(Class<?> clazz, BeanBoxContext context) {
		if (BeanBox.class.isAssignableFrom(clazz))
			...
			if (BeanBox.class.isAssignableFrom(newClass)) {
				...
				printAndThrowError(null, "BeanBox getBox error! class named with identity \"" + context.boxIdentity + "\" but is not a BeanBox class, class=" + className);
		}
	...
	private static <T> T createBeanOrBoxInstance(Class clazz, BeanBoxContext context) {
		try {
			...
			printAndThrowError(null, "BeanBox createBeanOrBoxInstance error: no 0 parameter constructor found! boxClass=" + clazz);
		} catch (Exception e) {
	...
	private static Object getProxyBean(Class clazz, CopyOnWriteArrayList<Advisor> advisorList) {// use CGLib create proxy bean, if advice set for this class
	...
	// If found advice for this class, use CGLib to create proxy bean, CGLIB is the only way to create proxy to make source code simple.
	...
	public static class BeanBoxContext {
		...
		protected CopyOnWriteArrayList<Advisor> advisorList = new CopyOnWriteArrayList<Advisor>();// Advisors stored in context
		protected HashMap<String, Object> signletonCache = new HashMap<String, Object>();// Singleton instance cached in context
		protected CopyOnWriteArrayList<Class<?>> configClassList = new CopyOnWriteArrayList<Class<?>>();// Global configurations
		protected ConcurrentHashMap<String, Method> preDestoryMethodCache = new ConcurrentHashMap<String, Method>();// preDestory methods cache
		...
		public void setAOPAround(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "AROUND", true));
		}

		// No explain, as method name said
		public void setAOPBefore(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "BEFORE", true));
		}

		public void setAOPAfterReturning(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "AFTERRETURNING", true));
		}

		public void setAOPAfterThrowing(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "AFTERTHROWING", true));
		}

		public void setAspectjAround(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "AROUND", false));
		}

		public void setAspectjBefore(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "BEFORE", false));
		}

		public void setAspectjAfterReturning(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "AFTERRETURNING", false));
		}

		public void setAspectjAfterThrowing(String classNameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceAroundMethodName) {
			advisorList.add(new Advisor(classNameReg, methodNameReg, adviceBeanBox, adviceAroundMethodName, "AFTERTHROWING", false));
		}
	}
	...
	private static class ProxyBean implements MethodInterceptor {// ProxyBean created by CGLib
		...
		public Object intercept(Object obj, Method method, Object[] args, MethodProxy cgLibMethodProxy) throws Throwable {
			if (myAdvisors.size() > 0 && myAdvisors.get(0).match(obj.getClass().getName(), method.getName()))
				return new AdviceCaller(this, obj, method, args, cgLibMethodProxy, myAdvisors).callNextAdvisor();// Start a advice chain call
			else
	...
	private static class Advisor {// Advisor = Advice + Point-cut
		...
		Advisor(String classnameReg, String methodNameReg, BeanBox adviceBeanBox, String adviceMethodName, String adviceType, boolean isAOPAlliance) {
			if (BeanBox.isEmptyStr(classnameReg) || BeanBox.isEmptyStr(methodNameReg) || adviceBeanBox == null || BeanBox.isEmptyStr(adviceMethodName))
				throw new AssertionError("BeanBox create Advisor error! ClassNameReg:" + classnameReg + " methodNameReg:" + methodNameReg + " beanbox:" + adviceBeanBox + " aroundMethodName:"
						+ adviceMethodName);
		...
		protected boolean match(String beanClassName, String methodName) {
			...
			if (i > 0)
				beanClassName = beanClassName.substring(0, i);
			return Pattern.compile(classnameReg).matcher(beanClassName).matches() && Pattern.compile(methodNameReg).matcher(methodName).matches();
		}
	}
	...
	private static class AdviceCaller {
		...
		protected AdviceCaller(Object proxyBean, Object target, Method method, Object[] arguments, MethodProxy cgLibMethodProxy, CopyOnWriteArrayList<Advisor> myAdvisors) {
			...
			if (currentAdvisorIndex == 0 || advisor.match(target.getClass().getName(), method.getName())) {
				...
				if (advisor.isAOPAlliance) {// AOP alliance type advice
					if ("AROUND".equals(advisor.adviceType)) {
						// public Object doAround(MethodInvocation caller) throws Throwable, AOP alliance & Spring's around advice
						Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { MethodInvocation.class });
						...
						Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { Method.class, Object[].class, Object.class });
						...
						Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { Object.class, Method.class, Object[].class, Object.class });
						...
						try {
							...
							Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { Method.class, Object[].class, Object.class, Exception.class });
					...
					if ("AROUND".equals(advisor.adviceType)) {
						...
						Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { ProceedingJoinPoint.class });
						...
						Object result = callNextAdvisor();
						Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { JoinPoint.class, Object.class });
						...
						try {
							...
							Method m = advice.getClass().getMethod(advisor.adviceMethodName, new Class[] { JoinPoint.class, Exception.class });
	...
	private static class AopAllianceInvocation implements MethodInvocation {// MethodInvocation interface detail see aopalliance doc
	...
	private static class AspectjJoinPoint implements JoinPoint {// JoinPoint interface detail see Aspectj doc
		...
		protected AspectjJoinPoint(Object proxyBean, Object target, Method method, Object[] arguments, AdviceCaller caller) {
			this.proxy = proxyBean;
			this.target = target;
			this.method = method;
			this.arguments = arguments;
	...
	private static class AspectjProceedingJoinPoint extends AspectjJoinPoint implements ProceedingJoinPoint {// Detail see Aspectj doc
		protected AspectjProceedingJoinPoint(Object proxyBean, Object target, Method method, Object[] arguments, AdviceCaller caller) {
	...
	/**
	 * Inject BeanBox to a field with @Inject annotation <br/>
	 * 1) A.class{ @InjectBox(B.class) C c; ...} will find BeanBox class B and inject into c, if B is not beanBox, will find static class "B$CBox.class" inside of B <br/>
	 * 2) A.class{ @InjectBox C c; ...} will BeanBox follow this order: CBox.class in same package of C, "A$CBox.class", "ABox$CBox.class", CBox in globalConfigs
	 */
	...
	public static @interface InjectBox {
		...
		public boolean required() default true;// if set to false and no BeanBox found, do nothing. Otherwise throw a Error
	}
